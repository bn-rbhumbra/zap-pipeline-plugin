...
public class ZapDriverImpl implements ZapDriver {
	...
	private JSONObject zapApi(String apiUrl, Map<String, String> params) throws ZapExecutionException {
		try {
			...
			Unirest.setTimeouts(60000, 15000);
	...
	public void zapCrawlerSuccess() throws InterruptedException, ZapExecutionException {
		...
		while (status < ZapDriver.COMPLETED_PERCENTAGE) {
			if (OffsetDateTime.now().isAfter(startedTime.plusSeconds(timeoutSeconds))) {
				throw new ZapExecutionException(
						"ZAP Crawler failed to complete within the set timeout of " + timeoutSeconds + " seconds.");
			}
	 ...
	 * @param path - The path to load from
	 */
	public void importOpenApi(String path) throws ZapExecutionException {
		log("zap: Importing OpenAPI Specification from " + path);
		Map<String, String> arguments = Collections.singletonMap("file", path);

		JSONObject result = zapApi("openapi/action/importFile", arguments);

		verifyApiResultIsOk(result, "Request to import OpenAPI Specification returned a non-'OK' result.");
	}
	...
	public void startZapProcess(String zapHome, FilePath ws, Launcher launcher) throws IOException {
		...
		launcher.launch().stdout(launcher.getListener().getLogger()).stderr(launcher.getListener().getLogger()).cmds(cmd).pwd(ws)
				.start();
